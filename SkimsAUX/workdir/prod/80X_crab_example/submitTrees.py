...
def main():
    ...
    config.General.workArea = 'stopTrees'
    ...
    config.Data.outLFNDirBase = '/store/user/jsingera/Ewkinos/'
    ...
    for ijob, job in enumerate(jobs) :
        ...
        if 'ext1' in job:
          config.General.requestName += '_ext1'
        if 'ext2' in job:
          config.General.requestName += '_ext2'

        if 'Fast' in job:
            pyCfgParams = ['mcInfo=1', 'GlobalTag=80X_mcRun2_asymptotic_2016_miniAODv2_v0', 'specialFix=JEC', 'jecDBname=Spring16_25nsFastSimMC_V1', 'fastsim=1']
            inputFiles = ['Spring16_25nsFastSimMC_V1.db']
        else:
            if 'Summer16' in job:
